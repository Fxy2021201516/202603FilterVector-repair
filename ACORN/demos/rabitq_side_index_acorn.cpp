...
bool RabitQSideIndex::build(
    ...
    if (base_vectors == nullptr) {
        std::cerr << "[ACORN-RabitQ] build failed: null base vectors.\n";
        return false;
    }
    if (num_points == 0 || dim == 0) {
        std::cerr << "[ACORN-RabitQ] build failed: empty vectors.\n";
        return false;
    }
    if (total_bits < 1 || total_bits > 9) {
        std::cerr << "[ACORN-RabitQ] build failed: total_bits must be in [1, 9].\n";
        return false;
    }

    num_points_ = num_points;
    dim_ = dim;
    num_clusters_ = static_cast<std::size_t>(num_groups);
    total_bits_ = total_bits;
    ex_bits_ = total_bits_ - 1;

    if (point_to_group.size() != num_points_) {
        std::cerr << "[ACORN-RabitQ] build failed: point_to_group size mismatch.\n";
        return false;
    }
    if (num_clusters_ == 0) {
        std::cerr << "[ACORN-RabitQ] build failed: num_groups is zero.\n";
        return false;
    }
    ...
    if (!rotator_) {
        std::cerr << "[ACORN-RabitQ] build failed: cannot create rotator.\n";
        return false;
    }

    padded_dim_ = rotator_->size();
    if (padded_dim_ < dim_ || padded_dim_ % 64 != 0) {
        std::cerr << "[ACORN-RabitQ] build failed: invalid padded dimension.\n";
        return false;
    }
    ...
    std::vector<std::size_t> cluster_counts(num_clusters_, 0U);
    std::vector<float> rotated(padded_dim_, 0.0F);

    for (std::size_t point_id = 0; point_id < num_points_; ++point_id) {
        const std::uint32_t raw_group = point_to_group[point_id];
        if (raw_group == 0 || raw_group > num_groups) {
            std::cerr << "[ACORN-RabitQ] build failed: invalid group id.\n";
            return false;
        }
        const std::size_t cluster_id = static_cast<std::size_t>(raw_group - 1);
        cluster_ids_[point_id] = static_cast<std::uint32_t>(cluster_id);
        cluster_counts[cluster_id] += 1U;

        const float* vec = base_vectors + point_id * dim_;
    ...
    for (std::size_t cluster_id = 0; cluster_id < num_clusters_; ++cluster_id) {
        float* centroid = centroids_.data() + cluster_id * padded_dim_;
        const std::size_t count = cluster_counts[cluster_id];
        if (count == 0U) {
            continue;
        }
    ...
    for (std::size_t point_id = 0; point_id < num_points_; ++point_id) {
        const std::size_t cluster_id = static_cast<std::size_t>(cluster_ids_[point_id]);
        const float* vec = base_vectors + point_id * dim_;
        rotator_->rotate(vec, rotated.data());
...
bool RabitQSideIndex::save(const std::string& filename) const {
    if (!enabled_ || !rotator_) {
        std::cerr << "[ACORN-RabitQ] save skipped: side index is not enabled.\n";
        return false;
    }

    std::ofstream out(filename, std::ios::binary);
    if (!out.is_open()) {
        std::cerr << "[ACORN-RabitQ] save failed: cannot open " << filename << '\n';
        return false;
    }
    ...
    if (!write_binary(out, kRabitQMagic) || !write_binary(out, kRabitQVersion) ||
        ...
        std::cerr << "[ACORN-RabitQ] save failed: failed to write header.\n";
        return false;
    }

    out.write(reinterpret_cast<const char*>(cluster_ids_.data()),
              static_cast<std::streamsize>(cluster_ids_.size() * sizeof(std::uint32_t)));
    out.write(reinterpret_cast<const char*>(centroids_.data()),
              static_cast<std::streamsize>(centroids_.size() * sizeof(float)));
    out.write(reinterpret_cast<const char*>(bin_data_.data()),
              static_cast<std::streamsize>(bin_data_.size() * sizeof(std::uint8_t)));
    out.write(reinterpret_cast<const char*>(ex_data_.data()),
              static_cast<std::streamsize>(ex_data_.size() * sizeof(std::uint8_t)));
    if (!out.good()) {
        std::cerr << "[ACORN-RabitQ] save failed: failed to write payload.\n";
        return false;
    }

    rotator_->save(out);
    if (!out.good()) {
        std::cerr << "[ACORN-RabitQ] save failed: failed to write rotator state.\n";
        return false;
    }
...
bool RabitQSideIndex::load(const std::string& filename) {
    ...
    if (!in.is_open()) {
        std::cerr << "[ACORN-RabitQ] load failed: cannot open " << filename << '\n';
        return false;
    }
    ...
    if (!read_binary(in, magic) || !read_binary(in, version)) {
        std::cerr << "[ACORN-RabitQ] load failed: cannot read header.\n";
        return false;
    }
    if (magic != kRabitQMagic || version != kRabitQVersion) {
        std::cerr << "[ACORN-RabitQ] load failed: incompatible format.\n";
        return false;
    }
    ...
    if (!read_binary(in, num_points_) || !read_binary(in, dim_) ||
        ...
        std::cerr << "[ACORN-RabitQ] load failed: cannot read metadata.\n";
        return false;
    }

    if (num_points_ == 0 || num_clusters_ == 0 || padded_dim_ % 64 != 0) {
        std::cerr << "[ACORN-RabitQ] load failed: invalid metadata.\n";
        return false;
    }
    ...
    in.read(reinterpret_cast<char*>(cluster_ids_.data()),
            static_cast<std::streamsize>(cluster_ids_.size() * sizeof(std::uint32_t)));
    in.read(reinterpret_cast<char*>(centroids_.data()),
            static_cast<std::streamsize>(centroids_.size() * sizeof(float)));
    in.read(reinterpret_cast<char*>(bin_data_.data()),
            static_cast<std::streamsize>(bin_data_.size() * sizeof(std::uint8_t)));
    in.read(reinterpret_cast<char*>(ex_data_.data()),
            static_cast<std::streamsize>(ex_data_.size() * sizeof(std::uint8_t)));
    if (!in.good()) {
        std::cerr << "[ACORN-RabitQ] load failed: cannot read payload.\n";
        return false;
    }
    ...
    if (!rotator_ || rotator_->size() != padded_dim_) {
        std::cerr << "[ACORN-RabitQ] load failed: invalid rotator state.\n";
        return false;
    }
    rotator_->load(in);
    if (!in.good()) {
        std::cerr << "[ACORN-RabitQ] load failed: cannot read rotator state.\n";
        return false;
    }
...
std::uint64_t RabitQSideIndex::estimated_rotator_state_bytes() const {
    // Keep estimator strictly sizeof/data-buffer based.
    return 0;
}

std::uint64_t RabitQSideIndex::estimated_memory_bytes(bool include_rotator_state) const {
    if (!enabled_) {
        return 0;
    }

    std::uint64_t bytes = 0;
    bytes += static_cast<std::uint64_t>(cluster_ids_.size()) * sizeof(std::uint32_t);
    bytes += static_cast<std::uint64_t>(centroids_.size()) * sizeof(float);
    bytes += static_cast<std::uint64_t>(bin_data_.size()) * sizeof(std::uint8_t);
    bytes += static_cast<std::uint64_t>(ex_data_.size()) * sizeof(std::uint8_t);
...
bool RabitQSideIndex::init_query(const float* query, QueryContext& ctx, InitTiming* timing) const {
    ...
    rotator_->rotate(query, ctx.rotated_query.data());
    if (timing) {
    ...
    for (std::size_t cluster_id = 0; cluster_id < num_clusters_; ++cluster_id) {
        const float* centroid = centroids_.data() + cluster_id * padded_dim_;
        const float l2 = rabitqlib::euclidean_sqr(ctx.rotated_query.data(), centroid, padded_dim_);
...
float RabitQSideIndex::estimate_bin(std::uint32_t point_id, QueryContext& ctx, float* low_dist) const {
    ...
    const char* bin_ptr = reinterpret_cast<const char*>(bin_data_.data() + point_id * size_bin_data_);
...
float RabitQSideIndex::estimate_full(std::uint32_t point_id, QueryContext& ctx, float* low_dist) const {
    if (ex_bits_ == 0) {
    ...
    const float norm = ctx.q_to_centroids[cluster_id];
    const char* bin_ptr = reinterpret_cast<const char*>(bin_data_.data() + point_id * size_bin_data_);
    const char* ex_ptr = reinterpret_cast<const char*>(ex_data_.data() + point_id * size_ex_data_);
