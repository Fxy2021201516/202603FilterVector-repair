...
def generate_qps_recall_grid(all_plot_items, main_title, output_filename, font_sizes, plot_settings={}, numbering_offset_start=None):
    ...
    # 这决定了图例中标签的排列顺序
    default_legend_order = ["ACORN-1", "ACORN-γ","NaviX","pre-filtering", "UNG","UNG+", "SmartRoute", "SmartRoute+","SmartRoute++","SmartRoute+++"]
    ...
    default_drawing_order = ["UNG", "ACORN-γ", "ACORN-1","ACORN-γ-improved", "NaviX","UNG+","pre-filtering", "SmartRoute", "SmartRoute+""SmartRoute+","SmartRoute++","SmartRoute+++"]
    ...
    for i in range(n_rows * n_cols):
        ...
        if all_recalls:
            ...
            # --- 给右侧边框多留出 0% 的空白空间，防止星星被挤压 ---
            right_limit = plot_max_x_adj + 0.00 if plot_max_x_adj >= 1.0 else plot_max_x_adj
            ax.set_xlim(plot_min_x_adj, right_limit)
            
            ax.xaxis.set_major_locator(mticker.MultipleLocator(step))
        else:
            # 同样的，如果没有数据时，默认宽度也拓展到 1.03
